...
public class SQLConnection {
	...
	public boolean connectToDatabase( String url, String baseName, String user, String pswd ) throws SQLException{
		con = DriverManager.getConnection( 
				"jdbc:mysql://" + url + "/" + baseName+"?characterEncoding=utf-8",
				user,
				pswd );
		if( con.isClosed() )
			return false;
	...
	public int addLoginInfo( String uName, String uPswd ) throws SQLException{
		...
		pst = con.prepareStatement( "insert into LoginInfo (UName, Pswd, Email ) values (?,?,?)");
	...
	public ArrayList<Proposal> getProposalByUName( String uName ) throws SQLException{
		...
		if( rs.next() ){
			...
			rs = st.executeQuery( "select * from Proposal where WriterId=" + uId + " and FileId<>0" );
			while( rs.next() ){
	...
	public Proposal getProposalById( int pId ) throws SQLException{
		...
		rs = st.executeQuery( "select * from Proposal where FileId=" + pId );
	...
	public ArrayList<Proposal> getProposalByTitle( String title ) throws SQLException{
		ArrayList<Proposal> pList = new ArrayList<Proposal>();
		rs = st.executeQuery( "select * from Proposal where Title=" + title + " and FileId<>0" );
	...
	public ArrayList<Proposal> getProposalByStatus( char status ) throws SQLException{
		ArrayList<Proposal> pList = new ArrayList<Proposal>();
		rs = st.executeQuery( "select * from Proposal where Status=\'" + status + "\' and FileId<>0" );
	...
	public ArrayList<Proposal> getProposalForAll() throws SQLException{
		ArrayList<Proposal> pList = new ArrayList<Proposal>();
		st = con.createStatement();
		rs = st.executeQuery( "select * from Proposal where FileId<>0" );
		while( rs.next() ){
			Calendar upload = Calendar.getInstance();	upload.setTime( new Date( rs.getDate( "UploadDate" ).getTime() ) );
			Calendar deadline = Calendar.getInstance(); deadline.setTime( new Date( rs.getDate( "Deadline" ).getTime() ) );
	...
	public int addProposal( String uName, Calendar date, Calendar endline, String title, String content ) throws SQLException{
		rs = st.executeQuery( "select * from Proposal where Title=\"" + title + "\"" );
		if( rs.next() )
	...
	public ArrayList<Standard> getStandardByUId( int uId ) throws SQLException{
		ArrayList<Standard> pList = new ArrayList<Standard>();
		st = con.createStatement();
		rs = st.executeQuery( "select * from UserInfo where UserId=\"" + uId + "\"" );
	...
	public ArrayList<Standard> getStangardByStatus( char status ) throws SQLException{
		ArrayList<Standard> pList = new ArrayList<Standard>();
		rs = st.executeQuery( "select * from Standard where Status=\'" + status + "\'" );
	...
	public ArrayList<Standard> getSysStandardForAll() throws SQLException {
		ArrayList<Standard> pList = new ArrayList<Standard>();
		rs = st.executeQuery( "select * from Standard" );
		while( rs.next() ){
			Calendar upload = Calendar.getInstance();	upload.setTime( new Date( rs.getDate( "UploadDate" ).getTime() ) );
	...
	public ArrayList<User> chkUserForAll() throws SQLException{
		...
		ResultSet rs = st.executeQuery( "select * from UserInfo where Feature = 1 or Feature = 2 or Feature = 3 or Feature = 4" );
		while( rs.next() ){
			Calendar cBirthDay = Calendar.getInstance();
			cBirthDay.setTime( new Date( rs.getDate( "Birth" ).getTime() ) );
			uList.add ( new User(
					rs.getInt( "UserId" ),
					rs.getString( "Name" ),
...
//			return SQLConnection.USER_EXIST;
	...
	public ArrayList<Proposal> searchByTitle( String Info ) throws SQLException{
		ArrayList<Proposal> pList = new ArrayList<Proposal>();
		rs = st.executeQuery( "select * from dbgirl.proposal where Title like \'%" + Info + "%\'");
