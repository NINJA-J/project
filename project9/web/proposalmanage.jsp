...
<body>
<%
    ArrayList<Proposal> pList = con.getProposalByStatus( (char)(con.chkUserById(con.getLoginId((String)session.getAttribute("user_now"))).getFeature()-1));
            ...
            <div class="list-group" id="listgroup">
                ...
                <c:forEach var="row" items="${result2.rows}">
                    <c:if test="${row.Feature!=1}">
                        ...
                        <a href="proposalmanage.jsp" class="list-group-item active">提案管理</a>
        ...
        <div class="col-md-9">
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        ...
                        <tr>
                            ...
                            <td><% out.print( p.analysisStatus(p.getStatus())); %>				</td>
                                ...
                                <a href="detail.jsp?id=<%=p.getId()%>">详情</a>                       <%
                                    switch (con.chkUserById(con.getLoginId((String)session.getAttribute("user_now"))).getFeature()){
                                        case User.FEATURE_INDUSTRY_MANAGER:                                %>
                                            <a href="recommend.jsp?id=<%=p.getId()%>">推荐</a>            <%
                                            break;
                                        case User.FEATURE_COMMITTEE_MANAGER:                                %>
                                            <a href="recommend.jsp?id=<%=p.getId()%>">备案</a>            <%
                                            break;
                                        case User.FEATURE_SEMINAR_MANAGER:                                  %>
                                            <a href="recommend.jsp?id=<%=p.getId()%>">立案</a>            <%
                                            break;
                                    }                                                   %>
                        </tr>
                        <%
            ...
            <div>
                <ul class="pagination pull-right">
                    <li>
                        <a href="proposalmanage.jsp?page=<%=indexB%>" aria-label="Previous">
                        ...
                        for( int i = 1; i <= indexM; i++ ) {
                            if( i == pageIndex ) {                                          %>
                                ...
                                <li>                                                        <%
                                    ...
                                    <a href="proposalmanage.jsp?page=<%=i%>"><%=i%></a>
                                </li>                                                           <%
                        }                                                                   %>
                    <li>
                        <a href="proposalmanage.jsp?page=<%=indexF%>" aria-label="Next">
