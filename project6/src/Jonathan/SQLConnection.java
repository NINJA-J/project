...
public class SQLConnection {
	...
	public Standard getStandardById( int sId ) throws SQLException{
		rs = st.executeQuery( "select * from Proposal where FileId=" + sId );
	...
	public ArrayList<Standard> getStandardByUId( int sId ) throws SQLException{
		ArrayList<Standard> sList = new ArrayList<Standard>();
		rs = st.executeQuery( "select * from Standard where WriterId=" + sId );

		while( rs.next() ){
			Calendar upload = Calendar.getInstance();	upload.setTime( new Date( rs.getDate( "UploadDate" ).getTime() ) );

			sList.add( new Standard(
					rs.getInt( "FileId" ),
					con2.chkUserById( rs.getInt( "WriterId" ) ),
					rs.getString( "Title" ),
					rs.getInt("ProposalId"),
